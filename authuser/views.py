...
def register(request):
    ...
    if request.method == 'POST':
        ...
        user = User(name=deserialize['name'], email=deserialize['email'],
                    password=hash_password, phone=deserialize['phone'], is_buyerseller=True)
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(email=deserialize['email']).exists():
            ...
            if check_password(deserialize['password'], user.password):
                user = authenticate(
                    request, email=deserialize['email'], password=deserialize['password'])
                refresh = RefreshToken.for_user(user)
                access = AccessToken.for_user(user)
                access.set_exp(lifetime=datetime.timedelta(days=30))
                refresh.set_exp(lifetime=datetime.timedelta(days=30))
                response = JsonResponse({'refreshToken': str(
                    refresh), 'accessToken': str(access), }, status=status.HTTP_200_OK)
                return response
            else:
...
def get_user_session(request):
    if request.method == 'GET':
        ...
        return JsonResponse({'email': user.email, 'name': user.name, 'phone': user.phone, 'is_admin':user.is_admin, 'is_buyerseller':user.is_buyerseller}, status=status.HTTP_200_OK)
...
def register_admin(request):
    if request.method == 'POST':
        ...
        hash_password = make_password(deserialize['password'])
        user = User(name=deserialize['name'], email=deserialize['email'],
                    password=hash_password, phone=deserialize['phone'], is_admin=True)
        # check user already exists on database
        if not (User.objects.filter(email=deserialize['email']).exists()):
            user.save()
            return JsonResponse({'message': 'User created successfully'}, status=status.HTTP_201_CREATED)
...
def get_user_profile(request):
    if request.method == 'GET':
        ...
        main_address = {}
        if Address.objects.filter(user_id=user, is_main=True).exists():
...
def set_main_address(request, id):
    if request.method == 'POST':
        main_address = Address.objects.get(user_id=request.user.email, is_main=True)
        new_main_address = Address.objects.get(id=id)
        main_address.is_main = False
        new_main_address.is_main = True
        main_address.save()
        new_main_address.save()
        return JsonResponse({'message': 'Main address updated successfully'}, status=status.HTTP_200_OK)
